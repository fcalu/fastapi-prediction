...
@app.post("/predecir")
def predecir_partido(data: PartidoRequest):
    try:
        ...
        # Historial completo del equipo local
        local_casa = df[df['home_team_name'] == data.equipo_local]
        local_visita = df[df['away_team_name'] == data.equipo_local]
        local_total = pd.concat([local_casa, local_visita])

        # Historial completo del equipo visitante
        visitante_casa = df[df['home_team_name'] == data.equipo_visitante]
        visitante_visita = df[df['away_team_name'] == data.equipo_visitante]
        visitante_total = pd.concat([visitante_casa, visitante_visita])
        ...
        def calcular_metricas(hist, equipo):
            ...
            return {
                "goles_favor": np.mean(goles),
                "xg": np.mean(xg),
                "tiros_arco": np.mean(tiros_arco),
                "posesion": np.mean(posesion),
        ...
        # Recomendación
        mayor_prob = max(prob_local, prob_empate, prob_visitante)
        if mayor_prob == prob_local:
            pred_final = f"Gana {data.equipo_local}"
        elif mayor_prob == prob_visitante:
            pred_final = f"Gana {data.equipo_visitante}"
        else:
            pred_final = "Empate probable"

        return {
            "historial_partidos": len(df[
                ((df['home_team_name'] == data.equipo_local) & (df['away_team_name'] == data.equipo_visitante)) |
                ((df['home_team_name'] == data.equipo_visitante) & (df['away_team_name'] == data.equipo_local))
            ]),
